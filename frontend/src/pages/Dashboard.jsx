...
import SeverityByCategoryChart from "../components/SeverityByCategoryChart";
...
export default function Dashboard() {
  ...
  const [categoryMagnitudeData, setCategoryMagnitudeData] = useState([]);
  ...
  const fetchCategoryMagnitude = async () => {
    try {
      const res = await api.get("/category-magnitude");
      setCategoryMagnitudeData(Array.isArray(res.data) ? res.data : []);
    } catch (err) {
      setCategoryMagnitudeData([]);
      if (!isNoDataError(err)) {
        console.error("Unable to load category magnitude data:", err);
      }
    }
  };
  ...
  const fetchScatterData = async () => {
    try {
      ...
      setScatterData([]);
      if (!isNoDataError(err)) {
        console.error("Unable to load scatter data:", err);
      }
    }
  };
    ...
    <div className="app-shell">
      ...
      <div className="content-body">
        ...
        <main className="main-area">
          ...
          <div style={{ display: "flex", gap: "12px", padding: "12px 0 0 0", flex: 1, minHeight: 0 }}>
            {/* Severity by Category Chart */}
            <div className="chart-panel-wrap" style={{ flex: 1, minHeight: 0 }}>
              <div className="chart-panel-header">
                <span className="chart-panel-title">Severity by Category</span>
                <button className="chart-menu-btn" aria-label="Chart options">
                  <MoreVertical style={{ width: 15, height: 15 }} />
                </button>
              </div>
              <div className="chart-body">
                {categoryMagnitudeData.length > 0 ? (
                  <SeverityByCategoryChart categoryMagnitudeData={categoryMagnitudeData} />
                ) : (
            ...
            <div className="chart-panel-wrap" style={{ flex: 1, minHeight: 0 }}>
              <div className="chart-panel-header">
                <span className="chart-panel-title">Session Analysis</span>
                <button className="chart-menu-btn" aria-label="Chart options">
                  <MoreVertical style={{ width: 15, height: 15 }} />
                </button>
              </div>
              <div className="chart-body">
                {scatterData.length > 0 ? (
                  <SessionAnalysisChart scatterData={scatterData} />
                ) : (
                  <div className="empty-chart">No session data available</div>
                )}
              </div>
            </div>
          </div>
